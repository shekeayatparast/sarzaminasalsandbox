...
// Convert Persian digits to English for validation/storage consistency
function persianToEnglishDigits(s: string): string {
...
export function isValidIranPhone(phone: string): boolean {
  ...
  // Accept both Persian and English digits
  const normalized = persianToEnglishDigits(phone.replace(/[\s\-()]/g, ""));
...
export function normalizeIranPhone(phone: string): string {
  // Convert Persian digits to English + normalize format
  const normalized = persianToEnglishDigits(phone.replace(/[\s\-()]/g, ""));
